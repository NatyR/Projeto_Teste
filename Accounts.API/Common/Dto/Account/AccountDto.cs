...
namespace Accounts.API.Common.Dto.Account
{
    public class AccountDto
    {
        ...
        public long IdConta { get { return this.Id; } }
        ...
        public void TratarCampos()
        {
            ...
            this.AddressComplement = (this.AddressComplement ?? "").Trim();
            this.PhoneNumber = (this.PhoneNumber ?? "").Trim();
            this.CostCenter = (this.CostCenter ?? "").Trim();
            this.Branch = (this.Branch ?? "").Trim();
            this.RegistrationNumber = (this.RegistrationNumber ?? "").Trim();
            this.MothersName = (this.MothersName ?? "").Trim();
            this.BirthState = (this.BirthState ?? "").Trim();
            this.BirthCity = (this.BirthCity ?? "").Trim();

            if (this.ZipCode != null && this.ZipCode.Length < 8)
            {
                while (this.ZipCode.Length < 8)
                {
